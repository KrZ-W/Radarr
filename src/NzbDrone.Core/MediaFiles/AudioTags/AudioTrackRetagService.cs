...
namespace NzbDrone.Core.MediaFiles.AudioTags
{
    public interface IAudioTrackRetagService
    {
        ...
        AudioTrackRetag Retag(MovieFile movieFile, Movie movie, bool hardlinkHint, bool manual);
    }
                                          ...
                                          IExecute<RetagAudioTracksCommand>
    {
        ...
        public void Handle(MovieFileImportedEvent message)
        {
            ...
            var movieFile = message.ImportedMovie;
            var movie = message.MovieInfo?.Movie ?? movieFile.Movie;
            var hardlinkHint = message.MovieInfo?.TransferMode == TransferMode.HardLink;

            try
            {
                Retag(movieFile, movie, hardlinkHint, false);
            }
            catch (Exception ex)
            {
                _logger.Warn(ex, "Audio track retag failed for '{0}'", movieFile);
            }
        }

        public void Execute(RetagAudioTracksCommand message)
        {
            var movieFile = _mediaFileService.GetMovie(message.MovieFileId);

            if (movieFile == null)
            {
                throw new ArgumentException($"Movie file {message.MovieFileId} does not exist");
            }

            var movie = movieFile.Movie ?? _movieService.GetMovie(movieFile.MovieId);

            var outcome = Retag(movieFile, movie, false, true);

            _logger.Info("Audio track retag of '{0}': {1}", movieFile, outcome?.Result ?? "nothing to do");
        }

        public AudioTrackRetag Retag(MovieFile movieFile, Movie movie, bool hardlinkHint, bool manual)
        {
            var path = LibraryPath(movieFile, movie);
            var remuxNonMkv = _configService.AudioTrackRetagNonMkvMode == AudioTrackRetagNonMkvMode.RemuxToMkv;
            var plan = AudioTrackRetagPlanner.Plan(movieFile, path, _configService.AudioLanguageVerificationConfidenceThreshold, remuxNonMkv);
            var mode = _configService.AudioTrackRetagHardlinkMode;

            switch (plan.SkipReason)
            {
                case AudioTrackRetagSkipReason.AlreadyDone:
                    _logger.Debug("Audio tracks of '{0}' were already retagged on {1}, skipping", path, movieFile.AudioTrackRetag.At);
                    return null;
                case AudioTrackRetagSkipReason.NotMkv:
                    ...
                    if (manual || movieFile.AudioLanguageVerification?.Any() == true)
                    {
                        _logger.Info("Audio tracks of '{0}' cannot be retagged: only Matroska files are supported", path);
                        return Record(movieFile, mode, AudioTrackRetagResult.SkippedContainer, plan, "only Matroska (.mkv) files can be retagged");
                    }
            ...
            if (!plan.Edits.Any())
            {
                ...
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, "no mismatched track has a language mkvpropedit can write");
            }

            if (!_diskProvider.FileExists(path))
            {
                _logger.Warn("Cannot retag audio tracks: '{0}' does not exist", path);
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, "file not found");
            }
            ...
            if (plan.Remux)
            {
                ...
                return RemuxToMkv(movieFile, movie, path, mode, plan);
            }
            ...
            if (mode == AudioTrackRetagHardlinkMode.Skip)
            {
                if (hardlink == HardlinkState.Linked)
                {
                    ...
                    return Record(movieFile, mode, AudioTrackRetagResult.SkippedHardlinked, plan, null);
                }
                ...
                if (hardlink == HardlinkState.Unknown)
                {
                    ...
                    return Record(movieFile, mode, AudioTrackRetagResult.SkippedHardlinked, plan, LinkCountUnavailable);
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, ex.Message);
            }

            _logger.Info("Retagged audio tracks of '{0}': {1}", path, string.Join(", ", plan.Edits.Select(e => $"a{e.StreamIndex} {e.From ?? "und"} -> {e.To}")));

            return VerifyAndRecord(movieFile, path, mode, plan);
        }
        ...
        /// </summary>
        private AudioTrackRetag RemuxToMkv(MovieFile movieFile, Movie movie, string path, AudioTrackRetagHardlinkMode mode, AudioTrackRetagPlan plan)
        {
            var originalContainer = Path.GetExtension(path).TrimStart('.').ToLowerInvariant();

            if (movie?.Path.IsNullOrWhiteSpace() != false)
            {
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, "remux needs the movie folder to compute the new relative path");
            }
            ...
            if (_diskProvider.FileExists(targetPath))
            {
                ...
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, $"'{Path.GetFileName(targetPath)}' already exists next to the file");
            }
            ...
            if (free.HasValue && free.Value < size)
            {
                ...
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, $"not enough free space in '{folder}' to remux the file ({free.Value.SizeSuffix()} free, {size.SizeSuffix()} needed)");
            }

            var sourceInfo = movieFile.MediaInfo?.RawStreamData.IsNotNullOrWhiteSpace() == true ? movieFile.MediaInfo : _videoFileInfoReader.GetMediaInfo(path);
            ...
            catch (Exception ex)
            {
                ...
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, $"remux failed: {ex.Message}");
            }
            ...
            movieFile.RelativePath = movie.Path.GetRelativePath(targetPath);
            movieFile.Path = targetPath;
            movieFile.Size = _diskProvider.GetFileSize(targetPath);
            movieFile.MediaInfo = newInfo;
            ...
            var outcome = Record(movieFile, mode, AudioTrackRetagResult.Done, plan, null, originalContainer);
            ...
            _eventAggregator.PublishEvent(new MovieRenamedEvent(movie, new List<RenamedMovieFile>
            {
                new RenamedMovieFile { MovieFile = movieFile, PreviousPath = previousPath, PreviousRelativePath = previousRelativePath }
            }));
        ...
        private AudioTrackRetag VerifyAndRecord(MovieFile movieFile, string path, AudioTrackRetagHardlinkMode mode, AudioTrackRetagPlan plan)
        {
            var mediaInfo = _videoFileInfoReader.GetMediaInfo(path);

            if (mediaInfo == null)
            {
                return Record(movieFile, mode, AudioTrackRetagResult.Failed, plan, "the file could not be probed after the edit");
            }
        ...
        private AudioTrackRetag Record(MovieFile movieFile, AudioTrackRetagHardlinkMode mode, string result, AudioTrackRetagPlan plan, string error, string remuxedFrom = null)
        {
            ...
            movieFile.AudioTrackRetag = outcome;
            _mediaFileService.Update(movieFile);

            return outcome;
        }

        private static string LibraryPath(MovieFile movieFile, Movie movie)
        {
            if (movie?.Path.IsNotNullOrWhiteSpace() == true && movieFile.RelativePath.IsNotNullOrWhiteSpace())
            {
                return Path.Combine(movie.Path, movieFile.RelativePath);
            }
