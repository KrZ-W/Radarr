...
{
    /// <summary>
    /// Pure decision logic: given a movie file and the verification threshold, which audio
    /// tracks get a new language tag. Only tracks whose detected language (at or above the
    /// threshold) maps to a different Radarr language than their tag are edited; tracks below
    ...
    public static class AudioTrackRetagPlanner
    {
        ...
        /// </summary>
        public static AudioTrackRetagPlan Plan(MovieFile movieFile, string path, double confidenceThreshold, bool remuxNonMkv = false)
        {
            if (movieFile.AudioTrackRetag?.IsDone == true)
            {
            ...
            var verification = movieFile.AudioLanguageVerification;
