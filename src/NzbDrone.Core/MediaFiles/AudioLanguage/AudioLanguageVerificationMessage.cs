...
namespace NzbDrone.Core.MediaFiles.AudioLanguage
{
    ...
    {
        public static string Describe(LocalMovie localMovie, Language wantedLanguage)
        {
            if (localMovie?.AudioLanguageVerification == null || localMovie.AudioLanguageVerification.Count == 0)
            {
                return null;
            }

            switch (localMovie.AudioLanguageTrigger)
            {
            ...
            var detected = string.Join(", ", localMovie.AudioLanguageVerification.Select(Format));
