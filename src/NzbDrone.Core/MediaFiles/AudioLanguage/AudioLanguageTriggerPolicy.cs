...
namespace NzbDrone.Core.MediaFiles.AudioLanguage
{
    public class AudioLanguageTriggerInput
    {
        ...
        public Language ProfileLanguage { get; set; } = Language.Any;
        public Language OriginalLanguage { get; set; } = Language.Unknown;

        /// <summary>True when any custom format used by the profile has a Language condition.</summary>
        ...
        /// <summary>Caller-evaluated: with the current tags the import-time language check or a language CF would reject the file (or drop it below MinFormatScore).</summary>
    ...
    public class AudioLanguageTriggerPolicy
    {
        public AudioLanguageTriggerDecision Evaluate(AudioLanguageTriggerInput input)
        {
            ...
            // Fast path: nothing in the profile cares about language.
            if (input.ProfileLanguage == Language.Any && !input.HasLanguageCustomFormat)
            {
                return AudioLanguageTriggerDecision.None;
            }
            ...
            var missingClaims = claimed
                .Where(l => input.Tracks.All(t => t.TaggedLanguage != l))
                .ToList();
        ...
        /// <summary>Profile language (Original resolved through the movie) plus the languages positive language CFs ask for.</summary>
        public static List<Language> ExpectedLanguages(AudioLanguageTriggerInput input)
        {
            var expected = new List<Language>();

            if (input.ProfileLanguage == Language.Original)
            {
            ...
            else if (IsConcrete(input.ProfileLanguage))
            {
                expected.Add(input.ProfileLanguage);
            }
