...
namespace NzbDrone.Core.MediaFiles.MovieImport.Aggregation.Aggregators.Augmenters.Language
{
    ...
    public class AugmentLanguageFromAudioProbe : IAugmentLanguage
    {
        ...
        public AugmentLanguageResult AugmentLanguage(LocalMovie localMovie, DownloadClientItem downloadClientItem)
        {
            ...
            try
            {
                return Augment(localMovie, downloadClientItem);
            }
            catch (Exception ex)
            {
                _logger.Warn(ex, "Audio language verification failed for '{0}', importing on existing evidence", localMovie.Path);
                return null;
            }
        }

        private AugmentLanguageResult Augment(LocalMovie localMovie, DownloadClientItem downloadClientItem)
        {
            ...
            if (localMovie.ExistingFile || localMovie.MediaInfo == null || localMovie.Movie?.QualityProfile == null)
            {
                return null;
            }

            var tracks = _layoutReader.Read(localMovie.MediaInfo);
            ...
            var profile = localMovie.Movie.QualityProfile;
            var originalLanguage = localMovie.Movie.MovieMetadata?.Value?.OriginalLanguage ?? Languages.Language.Unknown;
            ...
            var input = new AudioLanguageTriggerInput
            {
                ...
                ClaimedLanguages = GetClaimedLanguages(localMovie, downloadClientItem),
                Tracks = tracks,
                ProfileLanguage = profile.Language ?? Languages.Language.Any,
                OriginalLanguage = originalLanguage,
                HasLanguageCustomFormat = hasLanguageFormat,
                CustomFormatLanguages = formatLanguages,
                ImpendingRejection = WouldBeRejected(localMovie, evidence, hasLanguageFormat),
                ReleaseGroup = GetReleaseGroup(localMovie),
            ...
            var packKey = downloadClientItem?.DownloadId.IsNotNullOrWhiteSpace() == true
                ...
                : localMovie.Path.GetParentPath() ?? localMovie.Path;
            ...
            _logger.Debug("Audio language verification triggered ({0}) for '{1}', probing audio stream(s) {2}", decision.Trigger, localMovie.Path, string.Join(", ", decision.StreamIndexes));
            ...
            foreach (var streamIndex in decision.StreamIndexes)
            {
                ...
                {
                    result = _probe.Probe(localMovie.Path, streamIndex, localMovie.MediaInfo.RunTime);
            ...
            localMovie.AudioLanguageTrigger = decision.Trigger;
            localMovie.AudioLanguageVerification = verifications;
            ...
            {
                _logger.Debug("No audio language probe of '{0}' reached the confidence threshold {1:0.00}, keeping the tagged languages", localMovie.Path, threshold);
                return null;
            }
        ...
        private List<Languages.Language> GetClaimedLanguages(LocalMovie localMovie, DownloadClientItem downloadClientItem)
        {
            var claimed = new List<Languages.Language>();

            claimed.AddRange(localMovie.FileMovieInfo?.Languages ?? new List<Languages.Language>());
            claimed.AddRange(localMovie.FolderMovieInfo?.Languages ?? new List<Languages.Language>());
            claimed.AddRange(localMovie.DownloadClientMovieInfo?.Languages ?? new List<Languages.Language>());
        ...
        /// <summary>Mirrors the import-time LanguageSpecification and MinimumCustomFormatScoreSpecification on the tagged languages only.</summary>
        private bool WouldBeRejected(LocalMovie localMovie, List<Languages.Language> evidence, bool hasLanguageFormat)
        {
            var profile = localMovie.Movie.QualityProfile;
            var wanted = profile.Language ?? Languages.Language.Any;

            if (wanted == Languages.Language.Original)
            {
                var original = localMovie.Movie.MovieMetadata?.Value?.OriginalLanguage;

                if (original != null && original != Languages.Language.Unknown && !evidence.Contains(original))
                {
                    return true;
                }
            }
            else if (wanted != Languages.Language.Any && !evidence.Contains(wanted))
            {
                return true;
            }
            ...
            var previous = localMovie.Languages;
            ...
            {
                _logger.Debug(ex, "Unable to pre-compute the custom format score for '{0}'", localMovie.Path);
                return false;
            }
            finally
            {
                localMovie.Languages = previous;
            }
        }

        private static string GetReleaseGroup(LocalMovie localMovie)
        {
            return localMovie.ReleaseGroup.IsNotNullOrWhiteSpace()
                ? localMovie.ReleaseGroup
                : localMovie.FileMovieInfo?.ReleaseGroup ?? localMovie.FolderMovieInfo?.ReleaseGroup ?? localMovie.DownloadClientMovieInfo?.ReleaseGroup;
        }
    }
}
