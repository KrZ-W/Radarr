...
{
    [TestFixture]
    public class add_audio_track_retag_to_movie_filesFixture : MigrationTest<add_audio_track_retag_to_movie_files>
    {
        [Test]
        public void should_add_nullable_column_and_keep_existing_rows()
        {
            ...
            var rows = db.Query<MovieFile246>("SELECT \"Id\", \"AudioTrackRetag\" FROM \"MovieFiles\"").ToList();

            rows.Should().HaveCount(1);
            rows.First().AudioTrackRetag.Should().BeNull();
        }

        private class MovieFile246
        {
