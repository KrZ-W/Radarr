...
namespace NzbDrone.Core.Test.MediaFiles.AudioTags
{
    ...
    {
        private const string Mkv = "/movies/Movie (2020)/Movie (2020).mkv";
        ...
        private static MovieFile File(params AudioLanguageVerification[] tracks)
        {
            return new MovieFile { AudioLanguageVerification = tracks.Length == 0 ? null : new List<AudioLanguageVerification>(tracks) };
        }
        ...
        {
            AudioTrackRetagPlanner.Plan(File(Track(0, "eng", "fr")), "/movies/Movie (2020)/Movie (2020).mp4", Threshold).SkipReason.Should().Be(AudioTrackRetagSkipReason.NotMkv);
        ...
        {
            var plan = AudioTrackRetagPlanner.Plan(File(Track(0, "eng", "fr")), "/movies/Movie (2020)/Movie (2020).mp4", Threshold, remuxNonMkv: true);
        ...
        {
            AudioTrackRetagPlanner.Plan(File(Track(0, "fre", "fr")), "/movies/Movie (2020)/Movie (2020).mp4", Threshold, remuxNonMkv: true).SkipReason.Should().Be(AudioTrackRetagSkipReason.NoMismatch);
            AudioTrackRetagPlanner.Plan(File(), "/movies/Movie (2020)/Movie (2020).mp4", Threshold, remuxNonMkv: true).SkipReason.Should().Be(AudioTrackRetagSkipReason.NoVerificationRecord);
        }

        [Test]
        public void should_accept_upper_case_extension()
        {
            AudioTrackRetagPlanner.Plan(File(Track(0, "eng", "fr")), "/movies/Movie (2020)/Movie (2020).MKV", Threshold).ShouldRetag.Should().BeTrue();
        }
        ...
        public void should_skip_without_verification_record()
        {
            ...
            AudioTrackRetagPlanner.Plan(new MovieFile { AudioLanguageVerification = new List<AudioLanguageVerification>() }, Mkv, Threshold).SkipReason.Should().Be(AudioTrackRetagSkipReason.NoVerificationRecord);
        }
