...
namespace NzbDrone.Core.Test.MediaFiles.AudioTags
{
    ...
    public class AudioTrackRetagServiceFixture : CoreTest<AudioTrackRetagService>
    {
        ...
        private const string Mp4Path = "/movies/Movie (2020)/Movie (2020).mp4";
        private const string RemuxTempPath = "/movies/Movie (2020)/Movie (2020)" + AudioTrackRetagService.RemuxTempSuffix;
        ...
        public void should_keep_the_language_of_a_track_the_record_does_not_cover()
        {
            ...
            _movieFile.Languages = new List<Language> { Language.English, Language.French };
            _probedTags = new List<string> { "eng", "fre" };

            Import();

            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Done);
            _movieFile.Languages.Should().Equal(Language.English, Language.French);
        }
        ...
        {
            _movieFile.RelativePath = "Movie (2020).mp4";
            _movieFile.Size = 4_000_000_000L;
            _movieFile.MediaInfo = new MediaInfoModel { AudioLanguages = new List<string> { "eng" }, RawStreamData = SourceStreams, RunTime = TimeSpan.FromMinutes(90) };
            ...
            Mocker.GetMock<IDiskProvider>().Setup(d => d.GetParentFolder(Mp4Path)).Returns(MoviePath);
        ...
        private void VerifyOriginalUntouched()
        {
            ...
            Mocker.GetMock<IEventAggregator>().Verify(e => e.PublishEvent(It.IsAny<MovieRenamedEvent>()), Times.Never());
            _movieFile.RelativePath.Should().Be("Movie (2020).mp4");
            _movieFile.Size.Should().Be(4_000_000_000L);
        }
        ...
        public void should_not_start_ffmpeg_for_non_mkv_when_non_mkv_mode_is_skip()
        {
            ...
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.SkippedContainer);
            _movieFile.AudioTrackRetag.Remuxed.Should().BeFalse();
        }
        ...
        public void should_remux_non_mkv_and_update_the_file_record()
        {
            ...
            // record
            _movieFile.RelativePath.Should().Be("Movie (2020).mkv");
            _movieFile.Path.Should().Be(LibraryPath);
            _movieFile.Size.Should().Be(3_900_000_000L);
            _movieFile.MediaInfo.RawStreamData.Should().Be(RemuxedStreams);
            _movieFile.MediaInfo.AudioLanguages.Should().Equal("fre");
            _movieFile.Languages.Should().Equal(Language.French);
            Mocker.GetMock<IMediaFileService>().Verify(s => s.Update(_movieFile), Times.Once());

            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Done);
            _movieFile.AudioTrackRetag.Remuxed.Should().BeTrue();
            _movieFile.AudioTrackRetag.OriginalContainer.Should().Be("mp4");
            _movieFile.AudioTrackRetag.Tracks.Should().HaveCount(1);
            _movieFile.AudioTrackRetag.Error.Should().BeNull();

            // the same events the renamer raises
            Mocker.GetMock<IEventAggregator>().Verify(e => e.PublishEvent(It.Is<MovieFileRenamedEvent>(r => r.MovieFile == _movieFile && r.OriginalPath == Mp4Path && r.Movie == _movie)), Times.Once());
            Mocker.GetMock<IEventAggregator>().Verify(e => e.PublishEvent(It.Is<MovieRenamedEvent>(r => r.Movie == _movie && r.RenamedFiles.Count == 1 && r.RenamedFiles[0].MovieFile == _movieFile && r.RenamedFiles[0].PreviousPath == Mp4Path && r.RenamedFiles[0].PreviousRelativePath == "Movie (2020).mp4")), Times.Once());
        }
        ...
        {
            GivenRemuxableMp4();
            Mocker.GetMock<IDiskProvider>().Setup(d => d.GetAvailableSpace(MoviePath)).Returns(3_000_000_000L);
        ...
        public void should_refuse_remux_when_the_mkv_path_is_taken()
        {
            ...
            VerifyOriginalUntouched();
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("already exists");
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        public void should_delete_temp_and_record_failed_when_ffmpeg_fails()
        {
            ...
            VerifyOriginalUntouched();
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("Subtitle codec 94213 is not supported");
            _movieFile.AudioTrackRetag.Remuxed.Should().BeFalse();
            _movieFile.MediaInfo.RawStreamData.Should().Be(SourceStreams);
            _movieFile.Languages.Should().Equal(Language.French);
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        public void should_delete_temp_and_keep_the_original_when_the_duration_differs()
        {
            ...
            VerifyOriginalUntouched();
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("lasts");
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        public void should_delete_temp_and_keep_the_original_when_a_stream_went_missing()
        {
            ...
            VerifyOriginalUntouched();
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("2 stream(s), 3 expected");
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        public void should_delete_temp_and_keep_the_original_when_the_tag_was_not_written()
        {
            ...
            VerifyOriginalUntouched();
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("still reports a0=eng");
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        public void should_restore_the_original_when_the_remux_swap_fails_after_parking_it()
        {
            ...
            _movieFile.RelativePath.Should().Be("Movie (2020).mp4");
            _movieFile.AudioTrackRetag.Result.Should().Be(AudioTrackRetagResult.Failed);
            _movieFile.AudioTrackRetag.Error.Should().Contain("boom");
            ExceptionVerification.ExpectedWarns(1);
        }
        ...
        {
            GivenRemuxableMp4();
            _movieFile.MediaInfo = new MediaInfoModel { AudioLanguages = new List<string> { "eng" } };
        ...
        {
            AudioTrackRetagService.RemuxTargetPath("/movies/M (2020)/M.2020.avi").Should().Be("/movies/M (2020)/M.2020.mkv");
            AudioTrackRetagService.RemuxTempPath("/movies/M (2020)/M.2020.avi").Should().Be("/movies/M (2020)/M.2020.krzw-remux.tmp.mkv");
        }
    }
}
