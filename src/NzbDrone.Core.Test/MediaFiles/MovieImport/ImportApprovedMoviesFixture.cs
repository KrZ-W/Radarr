...
namespace NzbDrone.Core.Test.MediaFiles.MovieImport
{
    ...
    public class ImportApprovedMoviesFixture : CoreTest<ImportApprovedMovie>
    {
        ...
        public void should_finalize_upgrade_after_committing_new_download()
        {
            ...
            Mocker.GetMock<IUpgradeMediaFiles>()
                  .Verify(v => v.FinalizeUpgrade(It.IsAny<MovieFileMoveResult>()), Times.Once());

            Mocker.GetMock<IUpgradeMediaFiles>()
                  .Verify(v => v.RollbackUpgrade(It.IsAny<MovieFileMoveResult>()), Times.Never());
        }
        ...
        public void should_rollback_upgrade_and_not_finalize_when_adding_new_file_fails()
        {
            ...
            Mocker.GetMock<IUpgradeMediaFiles>()
                  .Verify(v => v.RollbackUpgrade(It.IsAny<MovieFileMoveResult>()), Times.Once());

            Mocker.GetMock<IUpgradeMediaFiles>()
                  .Verify(v => v.FinalizeUpgrade(It.IsAny<MovieFileMoveResult>()), Times.Never());
        ...
        [Test]
        public void should_carry_audio_language_verification_to_the_movie_file()
        {
            ...
            _approvedDecisions.First().LocalMovie.AudioLanguageVerification = verification;

            Subject.Import(new List<ImportDecision> { _approvedDecisions.First() }, true);

            Mocker.GetMock<IMediaFileService>().Verify(v => v.Add(It.Is<MovieFile>(c => c.AudioLanguageVerification == verification)), Times.Once());
        }

        [Test]
        public void should_leave_audio_language_verification_null_when_nothing_was_probed()
        {
            _approvedDecisions.First().LocalMovie.AudioLanguageVerification = null;
