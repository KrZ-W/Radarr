...
> **Status:** stable · **Since:** `v6.3.0.10514+krzw.6` · **Surface:** Settings → Media Management → *Audio Language Verification* (advanced), `MovieFile.AudioLanguageVerification` (`GET /api/v3/moviefile`)

## What it does

When a download's audio-track language **tags** are not trustworthy, Radarr listens to a
short clip of each suspicious track with a self-hosted **Whisper** speech-recognition
server and uses the detected language instead of the tag. A file whose release name says
`FRENCH` but whose only track is tagged `eng` — and really is French — imports with
`Languages = French`, keeps its VFQ/French custom formats, and is not offered a "French"
upgrade it does not need. A file that has no French audio at all is still rejected, and the
...
French/Quebec releases are routinely muxed with the audio track tagged `eng` or `und`
(the mux tool's default), and some groups label the *subtitle* language as the audio
language in the release name. [Import-time Enforcement](import-time-enforcement.md) then
does its job and rejects the file as "not French" — correctly, on the evidence it has —
and the release lands in the blocklist. Rescuing those downloads meant running a script
that probed the audio with Whisper, retagged a staging copy and re-imported. Doing the
detection inside the import pipeline removes the script and, more importantly, makes the
decision *before* the file is rejected.
...
the same container Bazarr's Whisper provider uses. Radarr calls only
...
plenty for language identification. The server must be reachable from the Radarr
...
(`audioLanguageVerification*`). "Expected languages" are never hard-coded: they come from
the movie's quality profile — its **Language** (with *Original* resolved through the
movie) plus the languages asked for by positively scored **Language** conditions in the
profile's custom formats. French is the dataset this was built for, not a constant.
...
- **expectation** — profile Language, plus whether any custom format used by the profile
  has a Language condition and which languages the positive ones want.

Nothing is probed when the feature is off, the endpoint is empty, the file is already in
the library (rescan / existing file), MediaInfo is missing, **or the profile Language is
*Any* and no custom format in the profile has a Language condition** (fast path). Otherwise
the first rule that applies decides which tracks are sent:
...
| c | **Impending rejection** — with the tags as they are, the import-time Language check would reject the file, or (when the profile uses a language CF) its custom-format score would fall below *MinFormatScore* | every track not tagged with an expected language |
...
a Radarr language. The *Timeout* setting is one budget per track: ffmpeg gets at most a
...
the ordered list of `(codec, channels, language tag, title)` of the audio streams. All files
of a pack that share a layout reuse the first file's answer, so a 10-file download with one
layout costs one probe per triggered track, not ten. Failed probes are cached too, but only
for **15 minutes** (a down server costs one attempt per layout per quarter hour, and a
...
- Either way the per-track outcome is stored on the `LocalMovie` and copied to
  `MovieFile.AudioLanguageVerification` when the file is imported (migration 245 adds the
  column). It is written **at import only**: `RescanMovie`, `UpdateMediaInfo` and the
  existing-file path of a disk scan neither re-probe nor rewrite it.

### Rejection reason

When triggers a–c fired, the probe ran, and the file is still rejected by the import-time
Language check (or the MinFormatScore check while a language CF is in play), the rejection
message ends with e.g.:

```
File audio language English does not contain the profile's required language (French). Audio verified: no French track (detected en 0.97, en 0.94)
```
...
Branch `feature/audio-language-verification-master` (cut from `v6.3.0.10514`), merged into
`personal/all-features-master`. Key files:
...
`MediaFiles/MovieImport/Aggregation/Aggregators/Augmenters/Language/AugmentLanguageFromAudioProbe.cs`,
`Datastore/Migration/245_add_audio_language_verification_to_movie_files.cs`,
`Configuration/ConfigService.cs`, `Radarr.Api.V3/Config/MediaManagementConfig*`,
`frontend/src/Settings/MediaManagement/MediaManagement.tsx`. The rejection-text hunks live
in the import-enforcement specs (`MovieImport/Specifications/LanguageSpecification.cs`,
`MinimumCustomFormatScoreSpecification.cs`) on the aggregate and their own fix branches.
