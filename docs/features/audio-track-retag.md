...
> **Status:** stable · **Since:** `v6.3.0.10514+krzw.8` (remux of non-MKV files: `+krzw.11`) · **Surface:** Settings → Media Management → *Audio Language Verification* (advanced) → *Retag Audio Tracks* / *Hardlinked Files* / *Non-MKV Files*, `MovieFile.AudioTrackRetag` (`GET /api/v3/moviefile`), command `RetagAudioTracks`

## What it does

After a download is imported, Radarr rewrites the **language tag** of the MKV audio tracks
...
The outcome is stored on the movie file as `AudioTrackRetag`, the file's `MediaInfo` and
...
Verification fixes what *Radarr* thinks the file is. It does not fix what **Plex, Jellyfin,
Emby, Kodi or Bazarr** think: they read the track tags, not Radarr's opinion. A rescued
Quebec-French movie whose only track is still tagged `eng` shows up as "English" in Plex,
...
- [Audio Language Verification](audio-language-verification.md) must have run for the file:
  the retag **reads** `MovieFile.AudioLanguageVerification` (per track: `streamIndex`,
...
- `mkvpropedit` from [MKVToolNix](https://mkvtoolnix.download/). The fork's image installs
  ...
  way `ffmpeg` is (bundled binary next to Radarr first, then `PATH`). On other installs put
  `mkvpropedit` on `PATH`.
...
`AudioTrackRetagService` handles `MovieFileImportedEvent` and retags when **all** of these hold:

1. *Retag Audio Tracks* is enabled;
2. the event is the import of a **new download** (`NewDownload`). Files discovered by
   *Rescan Movie*, a refresh, or the existing-file path of a disk scan raise the same event
...
For each mismatched track the detected ISO 639-1 code is mapped through Radarr's language
...
cross-checks it against the file-naming map. A track whose detected language Radarr does not
...
`mkvpropedit` runs through Radarr's process provider with a plain argument list (never a
...
always lands on the right track. Then `MovieFile.MediaInfo` is replaced with the fresh probe and
`MovieFile.Languages` is reconciled with the verification result rather than rebuilt from raw
...
| **Copy then retag** | The file is copied next to itself (`<name>.mkv.krzw-retag.tmp`, same directory, permissions copied), the **copy** is retagged, and the copy is atomically renamed over the library path (where `rename(2)` is unavailable it is swapped in through a `<name>.mkv.krzw-retag.bak` of the original, which is restored if the swap fails). Free space in the movie folder is checked first (the file's size); on any failure — not enough space, copy error, `mkvpropedit` error — the temp file is deleted and the original is left untouched (`failed`). | None. The seed keeps its original bytes; the library file becomes an independent copy (link count 1), so you lose the disk-space saving of the hardlink for that file. |
...
1. **Preconditions.** `<stem>.mkv` must not already exist in the movie folder, and the folder
...
7. **Rename events.** Radarr raises the same `MovieFileRenamedEvent` + `MovieRenamedEvent`
...
Migration 246 adds `MovieFiles.AudioTrackRetag` (JSON), exposed read-only on
`GET /api/v3/moviefile` as `audioTrackRetag`:
...
detected language Radarr does not know, or one without an ISO 639-2 code). `error` carries the
...
```
POST /api/v3/command
{ "name": "RetagAudioTracks", "movieFileId": 123 }
```
...
- **Never on rescan.** Only the import of a download triggers it; *Rescan Movie*, *Refresh*
...
- [Radarr#11385](https://github.com/Radarr/Radarr/issues/11385) — *Support for External
  Audio Language Provider / AI-Assisted Tagging* (open, Needs Triage). The request covers
  detecting the language *and* using it; the detection half is
  [Audio Language Verification](audio-language-verification.md), this feature is the part that
  writes the result back into the file so other applications see it. **Fixed here** in that
  sense; upstream has never modified media files and is unlikely to.
- [Sonarr#8453](https://github.com/Sonarr/Sonarr/issues/8453) — the same request on Sonarr
  (closed *not planned*); the maintainers pointed to a post-import custom script calling
  ffmpeg/whisper. The Sonarr fork implements both halves natively.
- [Radarr#7584](https://github.com/Radarr/Radarr/issues/7584) — *When using "Refresh & Scan",
  ...
  (closed, support). Adjacent, not fixed: upstream only re-reads track languages when the
  MediaInfo schema changes, so a file retagged by an external tool keeps stale languages in
  Radarr. This feature avoids the problem for its own edits by re-probing the file and
...
Branch `feature/audio-track-retag-master`, stacked on
`feature/audio-language-verification-master` (it needs the verification record and the
`ReadAllBytes` disk-provider member; the bare `v6.3.0.10514` tag cannot compile it), merged
into `personal/all-features-master`. Key files: `MediaFiles/AudioTags/*`
...
`RetagAudioTracksCommand`), `Datastore/Migration/246_add_audio_track_retag_to_movie_files.cs`,
`IDiskProvider.GetHardLinkCount` (+ Mono `stat` implementation), `LocalMovie.TransferMode`
(set by `MovieFileMovingService`), `Configuration/ConfigService.cs`,
`Radarr.Api.V3/Config/MediaManagementConfig*`, `MovieFileResource`,
`frontend/src/Settings/MediaManagement/MediaManagement.tsx`, `Dockerfile` (on the aggregate:
